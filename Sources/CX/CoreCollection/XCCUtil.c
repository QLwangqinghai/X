...
static inline XIndex __XCCDoubleCircularBufferSplitMoveCounts(XIndex capacity, XIndex begin, XIndex dstBegin, XIndex length, XIndex * _Nonnull mcounts, XBool isBackward) {
...
void XCCUtilCircularBufferMoveForward(XUInt8 * _Nonnull buffer, XIndex capacity, XIndex offset, XIndex elementSize, XRange range, XIndex n) {
    ...
    XIndex mcounts[3] = {};
    XIndex sliceCount = __XCCDoubleCircularBufferSplitMoveCounts(capacity, begin, dstBegin, range.length, mcounts, false);
...
void XCCUtilCircularBufferMoveBackward(XUInt8 * _Nonnull buffer, XIndex capacity, XIndex offset, XIndex elementSize, XRange range, XIndex n) {
    ...
    XIndex dstBegin = (range.location + n + offset) % capacity;
    XIndex mcounts[3] = {};
    XIndex sliceCount = __XCCDoubleCircularBufferSplitMoveCounts(capacity, begin, dstBegin, range.length, mcounts, true);
...
//isBackward == true 时， 先移动后半部分
static inline XIndex __XCCDoubleCircularBufferSplitCopyCounts(XIndex capacity, XIndex begin, XIndex dstCapacity, XIndex dstBegin, XIndex length, XIndex * _Nonnull mcounts) {
    XIndex p = capacity - length;
    XIndex counts[2] = {};
    if (begin > p) {
        counts[0] = capacity - begin;
...
void XCCUtilCircularBufferCopy(XUInt8 * _Nonnull dstBuffer, XIndex dstCapacity, XIndex dstOffset, XIndex dstIndex, XUInt8 * _Nonnull buffer, XIndex capacity, XIndex offset, XIndex index, XIndex length, XIndex elementSize) {
    ...
    XIndex ccounts[3] = {};
    XIndex sliceCount = __XCCDoubleCircularBufferSplitCopyCounts(capacity, begin, dstCapacity, dstBegin, length, ccounts);
